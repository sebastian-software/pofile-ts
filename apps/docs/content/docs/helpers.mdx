...
Beyond standard ICU MessageFormat, pofile-ts supports additional `Intl` formatters:
...
These formatters require native `Intl` APIs. Check browser compatibility:
...
All APIs are part of the [Baseline](https://web.dev/baseline) standard, meaning they work across the latest versions of Chrome, Firefox, Safari, and Edge. For older browsers, pofile-ts provides a JSON fallback for `duration`.
