...
The plural rules are validated against **official CLDR 48 test data**. All major languages pass 100% of CLDR sample tests.

### How the Data is Stored

Instead of shipping the full CLDR data (~50KB+), pofile-ts uses a **variant system**:

- **16 distinct plural rule patterns** cover all worldwide plural forms
- Each of the **140+ supported locales** maps to one variant
- Total data size: **~3KB** (gzipped: ~1KB)
...
| 3     | one, two, other                  | Sorbian, Sami                                 |
| 3     | zero, one, other                 | Latvian                                       |
| 4     | one, two, few, other             | Slovenian                                     |
| 4     | one, few, many, other            | Russian, Ukrainian, Belarusian                |
| 4     | one, few, many, other            | Polish, Czech, Slovak, Lithuanian             |
| 5     | one, two, few, many, other       | Irish, Maltese                                |
| 6     | zero, one, two, few, many, other | Arabic, Welsh                                 |

The rules are derived from [CLDR 48 Plural Rules](https://www.unicode.org/cldr/charts/latest/supplemental/language_plural_rules.html).

### Coverage

✅ **100% CLDR-compliant** for all major languages:

- East Asian: Chinese, Japanese, Korean, Vietnamese, Thai, Indonesian
- Germanic: English, German, Dutch, Swedish, Danish, Norwegian
- Romance: French, Spanish, Portuguese, Italian, Catalan
- Slavic: Russian, Ukrainian, Polish, Czech, Slovak, Croatian, Serbian
- Baltic: Lithuanian, Latvian
- Other: Arabic, Hebrew, Turkish, Finnish, Hungarian, Greek

⚠️ **Approximate support** for some niche languages:

- Scottish Gaelic (gd), Breton (br): simplified rules, same category count
- Welsh (cy): uses Arabic-like 6 categories as approximation

### Known Limitations

**Decimal Numbers**: JavaScript cannot distinguish `10` from `10.0` (they are identical). CLDR's `v` operand (decimal digits) is only supported for true non-integers like `0.5` or `1.5`. This matches practical usage since gettext/PO files work with integer counts.

**Compact Notation**: CLDR's "many" category for Romance languages (French, Spanish, etc.) includes compact notation (e.g., "1M"). We only support the integer million case (`n % 1000000 = 0`), which is sufficient for gettext usage.
...
// All major languages are CLDR 48 compliant
...
//     other: [0.5, 1.5, 2.5]  // fractions only
//   }
...
The samples are validated against CLDR 48 test data to ensure they correctly trigger each plural category.
...
Works with all CLDR 48 locales, including complex Slavic and Arabic plurals:
...
Compile ICU messages to fast JavaScript functions. Instead of parsing ICU syntax at runtime, the compiler generates optimized functions that directly produce formatted strings — **6× faster than Lingui**, **5× faster than FormatJS**.
...
| **Catalog (200 msgs)** | ~1.5M msgs/s |         **8× faster** |             — |
