...
<Callout type="info">
  These are **pofile-ts extensions** to ICU MessageFormat. Other ICU implementations may not
  recognize these types. Use them when you control both parsing and compilation (e.g., in a
  single-codebase i18n setup).
</Callout>
...
| Format        | Intl API              | Baseline | Links                                                                                                                                                                                    |
| ------------- | --------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `{n, plural}` | `Intl.PluralRules`    | 2018     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) · [CanIUse](https://caniuse.com/intl-pluralrules)                               |
| `{n, number}` | `Intl.NumberFormat`   | 2017     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_numberformat)     |
| `{d, date}`   | `Intl.DateTimeFormat` | 2017     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_datetimeformat) |
| `{t, time}`   | `Intl.DateTimeFormat` | 2017     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_datetimeformat) |

**Extended Formats (pofile-ts):**

| Format          | Intl API                  | Baseline | Links                                                                                                                                                                                            |
| --------------- | ------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `{x, list}`     | `Intl.ListFormat`         | 2021     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/ListFormat) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_listformat)                 |
| `{x, ago}`      | `Intl.RelativeTimeFormat` | 2020     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/RelativeTimeFormat) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_relativetimeformat) |
| `{x, name}`     | `Intl.DisplayNames`       | 2021     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DisplayNames) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_displaynames)             |
| `{x, duration}` | `Intl.DurationFormat`     | 2025     | [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DurationFormat) · [CanIUse](https://caniuse.com/mdn-javascript_builtins_intl_durationformat)         |

All APIs are part of the [Baseline](https://web.dev/baseline) standard. The oldest required API (`Intl.DurationFormat`) reached Baseline in March 2025. For older browsers, pofile-ts provides a JSON fallback for `duration`.
...
Compile ICU messages to fast JavaScript functions. Instead of parsing ICU syntax at runtime, the compiler generates optimized functions that directly produce formatted strings — **4× faster than Lingui and FormatJS**.
...
Currency can be provided at runtime via `values.currency`:
...
price({ amount: 99.99 }) // → "99,99 $" (USD fallback)
```

This is useful when the currency varies per user or transaction. Formatters are cached per currency for optimal performance.

### Custom Format Styles

Register custom format styles with full `Intl` API options. This allows domain-specific formatting without changing ICU message syntax:

```typescript
const fn = compileIcu("{size, number, bytes} uploaded on {d, date, iso}", {
  locale: "en",

  // Custom number styles (Intl.NumberFormat options)
  numberStyles: {
  ...
  // Custom date styles (Intl.DateTimeFormat options)
  dateStyles: {
...
fn({ size: 1024, d: new Date() })
// → "1,024B uploaded on 12/16/2024"
```

Custom styles fall back to built-in styles when no match is found. This allows gradual migration — add custom styles as needed without breaking existing messages.

### Pre-configured Compiler (Factory Pattern)

For real-world apps, define your custom styles once and reuse them everywhere:
...
pofile-ts includes 50+ built-in styles for common formatting needs. No configuration required:

#### Number Styles

```typescript
// Compact notation (social media, dashboards)
compileIcu("{n, number, compact}", { locale: "en" })({ n: 1500 }) // → "1.5K"
compileIcu("{n, number, compact}", { locale: "en" })({ n: 2500000 }) // → "2.5M"
compileIcu("{n, number, compactLong}", { locale: "en" })({ n: 1500 }) // → "1.5 thousand"

// Fixed decimals
compileIcu("{n, number, decimal2}", { locale: "en" })({ n: 3.1 }) // → "3.10"
compileIcu("{n, number, decimal3}", { locale: "en" })({ n: 3.14 }) // → "3.140"

// Sign display
compileIcu("{n, number, signAlways}", { locale: "en" })({ n: 5 }) // → "+5"
compileIcu("{n, number, signExceptZero}", { locale: "en" })({ n: 0 }) // → "0"

// File sizes
compileIcu("{n, number, byte}", { locale: "en" })({ n: 1024 }) // → "1,024 B"
compileIcu("{n, number, kilobyte}", { locale: "en" })({ n: 512 }) // → "512 kB"
compileIcu("{n, number, megabyte}", { locale: "en" })({ n: 1.5 }) // → "1.5 MB"
compileIcu("{n, number, gigabyte}", { locale: "en" })({ n: 2 }) // → "2 GB"
compileIcu("{n, number, terabyte}", { locale: "en" })({ n: 1 }) // → "1 TB"

// Distance
compileIcu("{n, number, meter}", { locale: "en" })({ n: 100 }) // → "100 m"
compileIcu("{n, number, kilometer}", { locale: "en" })({ n: 5 }) // → "5 km"
compileIcu("{n, number, mile}", { locale: "en" })({ n: 3 }) // → "3 mi"

// Temperature
compileIcu("{n, number, celsius}", { locale: "en" })({ n: 22 }) // → "22°C"
compileIcu("{n, number, fahrenheit}", { locale: "en" })({ n: 72 }) // → "72°F"

// Weight & Volume
compileIcu("{n, number, kilogram}", { locale: "en" })({ n: 70 }) // → "70 kg"
compileIcu("{n, number, liter}", { locale: "en" })({ n: 1.5 }) // → "1.5 L"

// Duration units
compileIcu("{n, number, second}", { locale: "en" })({ n: 30 }) // → "30 sec"
compileIcu("{n, number, minute}", { locale: "en" })({ n: 5 }) // → "5 min"
compileIcu("{n, number, hour}", { locale: "en" })({ n: 2 }) // → "2 hr"
```
...
```typescript
// ISO-like
compileIcu("{d, date, iso}", { locale: "en" })({ d: date }) // → "12/15/2024"

// Components
compileIcu("{d, date, weekday}", { locale: "en" })({ d: date }) // → "Sunday"
compileIcu("{d, date, weekdayShort}", { locale: "en" })({ d: date }) // → "Sun"
compileIcu("{d, date, monthYear}", { locale: "en" })({ d: date }) // → "December 2024"
compileIcu("{d, date, monthDay}", { locale: "en" })({ d: date }) // → "Dec 15"
compileIcu("{d, date, dayMonthYear}", { locale: "en" })({ d: date }) // → "Dec 15, 2024"
```

#### Time Styles

```typescript
compileIcu("{t, time, hourMinute}", { locale: "en" })({ t: time }) // → "14:30"
compileIcu("{t, time, hourMinute24}", { locale: "en" })({ t: time }) // → "14:30"
compileIcu("{t, time, hourMinute12}", { locale: "en" })({ t: time }) // → "2:30 PM"
compileIcu("{t, time, hourMinuteSecond}", { locale: "en" })({ t: time }) // → "14:30:45"
compileIcu("{t, time, hour12}", { locale: "en" })({ t: time }) // → "2 PM"
```

#### List Styles

```typescript
compileIcu("{items, list}", { locale: "en" })({ items: ["A", "B", "C"] })
// → "A, B, and C"

compileIcu("{items, list, or}", { locale: "en" })({ items: ["A", "B", "C"] })
// → "A, B, or C"

compileIcu("{items, list, narrow}", { locale: "en" })({ items: ["A", "B", "C"] })
// → "A, B, C"

compileIcu("{items, list, short}", { locale: "en" })({ items: ["A", "B", "C"] })
// → "A, B, & C"
```

All built-in styles work with any locale and can be combined with custom styles.
...
| ---------------------- | ---------: | --------------------: | --------------------: |
| **Compilation**        | 409k ops/s |         **7× faster** |                     — |
| **Runtime**            | 792k ops/s |         **3× faster** |         **4× faster** |
| **Catalog (200 msgs)** |   ~1.35M/s |         **7× faster** |                     — |
