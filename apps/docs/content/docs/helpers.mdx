...
IcuNodeType.literal // 0 - Plain text
IcuNodeType.argument // 1 - {name}
IcuNodeType.number // 2 - {n, number}
IcuNodeType.date // 3 - {d, date}
IcuNodeType.time // 4 - {t, time}
IcuNodeType.select // 5 - {g, select, ...}
IcuNodeType.plural // 6 - {n, plural, ...} or {n, selectordinal, ...}
IcuNodeType.pound // 7 - # in plural
IcuNodeType.tag // 8 - <b>...</b>
```

For debugging, use `getNodeTypeName()` to convert numeric types to readable strings:

```typescript
import { getNodeTypeName, IcuNodeType } from "pofile-ts"

getNodeTypeName(6) // → "plural"
getNodeTypeName(IcuNodeType.select) // → "select"
getNodeTypeName(node.type) // → "argument"
```
