...
parseIcu("{days, relativeTime, day}") // "in 3 days" or "3 days ago"
parseIcu("{hours, relativeTime, hour short}") // "in 2 hr."

// Display names (Intl.DisplayNames)
parseIcu("{lang, displayNames, language}") // "en" → "English"
parseIcu("{country, displayNames, region}") // "DE" → "Germany"
parseIcu("{code, displayNames, currency}") // "EUR" → "Euro"
```
...
const relativeFn = compileIcu("{days, relativeTime, day}", { locale: "de" })
relativeFn({ days: -2 })
// → "vor 2 Tagen"

const displayFn = compileIcu("{lang, displayNames, language}", { locale: "de" })
displayFn({ lang: "en" })
// → "Englisch"
```
